...
from aksis.deploy.monitor import (
    PerformanceMonitor,
    MonitoringConfig,
    MonitoringError,
)
...
class TestPerformanceMonitor:
    ...
    def test_validate_config_invalid_max_log_size(self) -> None:
        ...
        with pytest.raises(
            MonitoringError, match="Maximum log size must be positive"
        ):
    ...
    def test_validate_config_invalid_backup_count(self) -> None:
        ...
        with pytest.raises(
            MonitoringError, match="Backup count cannot be negative"
        ):
            PerformanceMonitor(config)
    ...
    def test_get_system_metrics_memory(self, mock_swap, mock_memory):
        ...
        mock_swap.return_value = Mock(
            total=2147483648, used=1073741824, percent=50.0  # 2GB  # 1GB
        )
    ...
    def test_get_system_metrics_gpu(
    ...
    @patch("torch.cuda.memory_allocated")
    @patch("torch.cuda.memory_reserved")
    def test_get_model_metrics_gpu(
        self, mock_reserved, mock_allocated, mock_available
    ):
    ...
    def test_collect_metrics(self) -> None:
        ...
        with patch.object(
            ...
            self.monitor, "get_model_metrics"
        ) as mock_model_metrics:
    ...
    def test_get_metrics_summary_with_duration(self) -> None:
        ...
        with patch("time.time", return_value=current_time):
            ...
            self.monitor.metrics_history = [
                {
                    "system": {"timestamp": 995, "cpu_percent": 20.0}
                },  # 5 seconds ago
                {
                    "system": {"timestamp": 998, "cpu_percent": 30.0}
                },  # 2 seconds ago
