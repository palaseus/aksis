...
class TestDistributedTrainer:
    ...
    def test_wrap_model_distributed(self, mock_dist_available, mock_init_process_group):
    ...
    def test_sync_gradients_distributed(self, mock_is_initialized, mock_all_reduce, mock_world_size):
    ...
    def test_optimize_for_inference_no_jit(self) -> None:
        ...
        with patch('torch.jit.optimize_for_inference', side_effect=Exception("JIT not available")):
            result = self.trainer.optimize_for_inference(mock_model)
            assert result == mock_model
    ...
    def test_scaling_error_with_details(self) -> None:
        ...
        error = ScalingError("Test error", details={"gpu_count": 2, "batch_size": 32})
