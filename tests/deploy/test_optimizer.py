...
from aksis.deploy.optimizer import (
    ModelOptimizer,
    OptimizationConfig,
    OptimizationError,
)
...
class TestModelOptimizer:
    ...
    def test_validate_config_invalid_quantization(self) -> None:
        ...
        with pytest.raises(
            OptimizationError, match="Invalid quantization type"
        ):
    ...
    def test_quantize_model_static(
        self, mock_convert, mock_prepare, mock_qconfig
    ):
    ...
    def test_quantize_model_invalid_type(self) -> None:
        ...
        mock_model = Mock()
        with pytest.raises(
            OptimizationError, match="Unsupported quantization type"
        ):
    ...
    def test_export_to_onnx(self, mock_export):
        ...
        result = self.optimizer.export_to_onnx(
            mock_model, "test_model.onnx", input_shape=(1, 512)
        )
    ...
    def test_optimize_for_inference_trace_fallback(
        self, mock_trace, mock_script
    ):
    ...
    def test_compare_models(self) -> None:
        ...
        with patch.object(self.optimizer, "get_model_size") as mock_get_size:
            ...
            result = self.optimizer.compare_models(
                mock_original, mock_optimized
            )
    ...
    def test_optimize_model(
        ...
        result = self.optimizer.optimize_model(
            mock_model, "output.pt", save_onnx=True
        )
