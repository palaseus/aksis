...
class OptimizationConfig(BaseModel):
    ...
    onnx_export: bool = Field(
        False, description="Export model to ONNX format"
    )
...
class ModelOptimizer:
    ...
    def _validate_config(self) -> None:
        ...
        if self.config.quantization_type not in ["dynamic", "static", "qat"]:
            raise OptimizationError(f"Invalid quantization type: {self.config.quantization_type}")
        
        if self.config.target_device not in ["cpu", "cuda", "mobile"]:
            raise OptimizationError(f"Invalid target device: {self.config.target_device}")
        
        if self.config.compression_ratio <= 0 or self.config.compression_ratio >= 1:
            raise OptimizationError("Compression ratio must be between 0 and 1")
    ...
    def quantize_model(
        ...
        try:
            if qtype == "dynamic":
                ...
                raise OptimizationError(f"Unsupported quantization type: {qtype}")
            
            logger.info(f"Model quantized successfully with {qtype} quantization")
    ...
    def export_to_onnx(
        ...
        try:
            ...
            logger.info(f"Model exported to ONNX successfully at {output_path}")
            return True
    ...
    def get_model_size(self, model: nn.Module) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Model size: {total_params:,} parameters, {model_size_mb:.2f} MB")
            return size_info
    ...
    def compare_models(
        ...
        try:
            ...
            if "model_size_mb" in original_size and "model_size_mb" in optimized_size:
                compression_ratio = optimized_size["model_size_mb"] / original_size["model_size_mb"]
            else:
            ...
            if end_time:
                ...
                original_time = start_time.elapsed_time(end_time) / 1000.0  # Convert to seconds
            else:
            ...
            comparison = {
                ...
                "speedup_ratio": original_time / optimized_time if optimized_time > 0 else 0,
            }
            
            logger.info(f"Model comparison completed. Compression ratio: {compression_ratio:.2f}")
            return comparison
    ...
    def optimize_model(
        ...
        try:
            ...
            # Calculate compression ratio
            if "model_size_mb" in original_size and "model_size_mb" in final_size:
                compression_ratio = final_size["model_size_mb"] / original_size["model_size_mb"]
            ...
            if save_onnx or self.config.onnx_export:
                onnx_path = output_path.with_suffix('.onnx') if output_path else Path("optimized_model.onnx")
