...
class DockerManager:
    ...
    def _validate_config(self) -> None:
        ...
        if self.config.dockerfile_path != "Dockerfile" and not os.path.exists(
            self.config.dockerfile_path
        ):
            raise FileNotFoundError(
                f"Dockerfile not found: {self.config.dockerfile_path}"
            )

        if self.config.context_path != "." and not os.path.exists(
            self.config.context_path
        ):
            raise FileNotFoundError(
                f"Context path not found: {self.config.context_path}"
            )
    ...
    def build_image(self, force_rebuild: bool = False) -> bool:
        ...
        try:
            ...
            if not force_rebuild and self._image_exists():
                logger.info(
                    f"Image {self.config.image_name}:{self.config.tag} already exists"
                )
                return True
            ...
            cmd = [
                ...
                "build",
                "-t",
                f"{self.config.image_name}:{self.config.tag}",
                "-f",
                self.config.dockerfile_path,
                self.config.context_path,
            ]
    ...
    def stop_container(self, container_name: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Docker container {container_name} stopped successfully"
            )
            return True
    ...
    def remove_container(self, container_name: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Docker container {container_name} removed successfully"
            )
            return True
    ...
    def remove_image(
        self, image_name: Optional[str] = None, tag: Optional[str] = None
    ) -> bool:
        """Remove Docker image.
        ...
        try:
            ...
            logger.info(
                f"Docker image {image_name}:{tag} removed successfully"
            )
            return True
    ...
    def list_containers(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error during Docker list containers: {e}"
            )
            return []
